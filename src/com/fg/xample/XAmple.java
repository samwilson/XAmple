...
import com.fg.custnodes.*;

/**
 * A simple application demonstrates usage of the Swing-based XML editing
 * component {@link com.fg.xmleditor.FXDoubleView}. The component generates
 * document-specific GUI for each particular type of the XML document,
 * specified by the XML Schema. A User-Manual document, which is included
 * into XAmple XML Editor distribution package, provides detailed description 
 * of how to customize XML Schema documents with the editor-specific features, 
 * such as custom field editors, additional labeling, node-specific messages, etc.
 * <p></p>
 * The application creares an instance of the
 * {@link com.fg.custnodes.FFileDialog} and registers it with the
 * {@link com.fg.xmleditor.FXDoubleView} under "FileDialog" id, so it can be
 * used as a global field editor. (Actually there is no particular sense in
 * doing it since the dialog is not initialized with any specific data. It is
 * just an example). 
 *
  ...
  implements ActionListener, ItemListener, FXModelStatusListener
{
  ...
  JMenuItem mOpenXSD = new JMenuItem("Open XML Schema");
  JMenuItem mOpenXML = new JMenuItem("Open XML Document");
  JMenuItem mNewXML = new JMenuItem("New XML Document");
  JMenuItem mSaveXML = new JMenuItem("Save XML Document");
  ...
  public XAmple()
  {
    ...
    btnOpenXSD = new FHistoryButton(icoOpenXSD, "Open XML Schema",
      "Reopen XML Schema");
    ...
    btnOpenXML = new FHistoryButton(icoOpenXML, "Open XML Document",
      "Reopen XML Document");
    ...
    btnNewXML = new Btn(icoNewXML, "New XML Document");
    toolBar.add(btnNewXML);
    btnSaveXML = new Btn(icoSaveXML, "Save XML Document");
  ...
  void showXSDTitle()
  {
    setTitle("XAmple  -  " + xsdFileName + "  :  <noname>.xml");
  }

  void showXMLTitle()
  {
    setTitle("XAmple  -  " + xsdFileName + "  :  " + xmlFileName);
  }
  ...
  void saveRuntimeProperties()
  {
    ...
    try {
      ...
      props.store(out, "XAmple XML Editor runtime properties");
    } catch (Exception ex) {
      ex.printStackTrace();
    } finally {
...
//          dblView.stopEditing();
  ...
  boolean confirmation()
  {
    if (dblView.isDocChanged())
    {
      int option = JOptionPane.showConfirmDialog(this,
        "The XML Document has been changed. Do you like to save it?",
  ...
  void newXML()
  {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(this, "Can't open new XML Document",
  ...
  boolean loadXMLDocument(File file)
  {
    ...
    try {
      ...
      childHistory.put(file.getAbsolutePath());
      btnOpenXML.setItems(childHistory.items);
      dblView.showInfoMessage("XML Document: " + file.getAbsolutePath());
      return true;
    } catch (Exception ex) {
...
//      dblView.clear();
      setXMLEnabled(false);
      dblView.showErrorMessage("Error: " + ex.getMessage());
      JOptionPane.showMessageDialog(this, "Can't load the XML Document",
        "Error", JOptionPane.ERROR_MESSAGE);
      showXSDTitle();
      return false;
    }
